...
import VM2 from "../../vm/near-social-vm/src/lib/vm/vm";
import {
	deepCopy,
	deepEqual,
	ErrorFallback,
...
const computeSrcOrCode = (src, code, configs) => {
	...
	for (const config of configs || []) {
		if (srcOrCode?.src) {
			...
			let value =
				isObject(config?.redirectMap) && config.redirectMap[src];
			if (!value) {
				try {
					value =
						isFunction(config?.redirect) && config.redirect(src);
				} catch {}
			}
			if (isString(value)) {
				srcOrCode = { src: value };
...
export function Widget(props) {
	...
	useEffect(() => {
		const computedSrcOrCode = computeSrcOrCode(
			propsSrc,
			propsCode,
			configs
		);
